...
def stem_word(word: str) -> str:
    """
    Cache global para a função de stemming.
    ⚡ Bolt: Optimizes stemming by caching results across NeshTextProcessor instances and queries.
    """
    return PortugueseStemmer.stem(word)
...
class NeshTextProcessor:
    ...
    def __init__(self, stopwords: List[str] = None):
        self.stemmer = PortugueseStemmer()  # Mantido para retrocompatibilidade
        self.stopwords = set(stopwords) if stopwords else set()

    def normalize(self, text: str) -> str:
        """Remove acentos e minúsculas."""
        text = PortugueseStemmer._remove_accent(text.lower())
        return text
    ...
    def process(self, text: str) -> str:
        ...
        for w in words:
            ...
            # ⚡ Bolt: Uses the globally cached stemmer function
            stemmed = stem_word(w)
