...
# Performance: lru_cache for stemming words.
# Word stemming is an expensive operation called thousands of times per document.
# By caching the result of `stem` (and decoupling it from the class instance `self`
# to avoid memory leaks), we significantly reduce CPU usage during FTS indexing and querying.
@lru_cache(maxsize=8192)
def _cached_stem(word: str) -> str:
    """Module-level cached stemmer function."""
    word = word.lower()
...
class PortugueseStemmer:
    ...
    def stem(self, word: str) -> str:
        """Processa a palavra e extrai sua raiz lexical, usando cache para performance."""
