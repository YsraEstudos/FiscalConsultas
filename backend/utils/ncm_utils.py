...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # ⚡ Bolt: Generator comprehension is faster than re.sub for filtering specific chars
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # ⚡ Bolt: "".join(s.split()) is faster than re.sub for removing whitespace
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt: chained string methods replace re.split for significant performance boost
