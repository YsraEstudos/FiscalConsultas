...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimization: Generator comprehension is significantly faster than regex re.sub for strict digit stripping
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimization: using string split and join is much faster than re.sub for removing whitespace
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimization: Replacing delimiters with spaces and using split() is significantly faster than re.split
