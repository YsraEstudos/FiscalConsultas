...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimized: generator comprehension with string inclusion is faster than re.sub for removing non-digits
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimized: "".join(raw.split()) is faster than re.sub for removing whitespace
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimized: replace and split is significantly faster than re.split for simple delimiters
