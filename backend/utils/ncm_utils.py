...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # ⚡ Bolt optimization: Generator comprehension is faster than re.sub for strict numeric filtering
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # ⚡ Bolt optimization: .split() and "".join() is faster than re.sub for removing all whitespace
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt optimization: Chained replace and split is drastically faster than re.split for simple multi-character delimiters
