...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Performance optimization: generator comprehension is faster than regex overhead
    # and strictly safer against unicode digits than str.isdigit()
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Performance optimization: str.split/join is significantly faster for whitespace removal than regex
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Performance optimization: chained replace + split() is highly optimized compared to re.split()
