...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimização Bolt: Uso de generator ao invés de regex para remoção estrita não-numérica é mais rápido e seguro
    s = (ncm or "").strip()
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimização Bolt: Uso de "".join(str.split()) é mais rápido que regex \s+ para colapsar/remover whitespace
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimização Bolt: Normalizar delimitadores via .replace() seguido por .split() é significativamente mais rápido que re.split()
