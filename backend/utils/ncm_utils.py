...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Performance optimization: use generator comprehension instead of re.sub for faster strict digit filtering
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Performance optimization: use "".join(split()) instead of re.sub(r"\s+", "", ...) for faster whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Performance optimization: chained replaces then split() is faster than re.split with multiple delimiters
