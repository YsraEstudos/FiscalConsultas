...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Bolt: using generator comprehension instead of re.sub for faster digit extraction
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Bolt: "".join(raw.split()) is significantly faster than re.sub(r"\s+", "", raw)
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Bolt: chained .replace().split() is significantly faster than re.split(r"[;,\s]+")
