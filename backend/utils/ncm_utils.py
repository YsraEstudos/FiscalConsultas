...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimized: generator comprehension with string check is faster than re.sub for ASCII digits
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimized: "".join(raw.split()) is faster than re.sub(r"\s+", "", raw)
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimized: chained replace and split is faster than re.split with multiple delimiters
