...
def clean_ncm(ncm: str) -> str:
    ...
    # Generator comprehension with strict string matching `c in "0123456789"`
    # avoids `re.sub` compilation overhead and is faster than `.isdigit()`
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    raw = (ncm or "").strip()
    # ⚡ Bolt Performance Optimization:
    # `"".join(raw.split())` is significantly faster than `re.sub(r"\s+", "", raw)`
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt Performance Optimization:
    # Using chained `.replace()` to normalize delimiters to spaces followed by `.split()`
    # is significantly faster than `re.split(r"[;,\s]+", ...)`
