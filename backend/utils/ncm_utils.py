...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # ⚡ Bolt: Using generator comprehension is faster than re.sub(r"[^0-9]", "")
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # ⚡ Bolt: Using split and join is faster than re.sub(r"\s+", "", raw)
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt: Using replace and split is significantly faster than re.split
