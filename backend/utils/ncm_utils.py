...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Bolt: Replaced re.sub with generator comprehension for faster non-numeric stripping
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Bolt: Replaced re.sub with "".join(split()) for faster whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Bolt: Replaced re.split with chained replace.split() for faster multi-delimiter splitting
    return [p for p in (query or "").replace(";", " ").replace(",", " ").split() if p]
