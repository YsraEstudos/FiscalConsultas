...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Opt: Utiliza generator para strip de caracteres numéricos (mais rápido que re.sub)
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Opt: str.replace e join evitam regex para remoção de espaços e são mais rápidos
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Opt: encadeamento de replaces seguido de split para máxima velocidade comparado a regex
