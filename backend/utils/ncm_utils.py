...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimized: Generator comprehension is faster than re.sub for ASCII strict matching
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimized: str.split() + join is faster than re.sub for whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimized: Chained replace + split is significantly faster than re.split with multiple delimiters
