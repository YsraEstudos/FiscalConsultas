...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # ⚡ Bolt: Fast string comprehension instead of regex for character filtering
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # ⚡ Bolt: Fast string split/join instead of regex for whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt: Fast string replace and split instead of regex for multi-delimiter splitting
    q = query or ""
    return q.replace(";", " ").replace(",", " ").split()
