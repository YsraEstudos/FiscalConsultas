...
def clean_ncm(ncm: str) -> str:
    ...
    """
    s = (ncm or "").strip()
    # Performance optimization: generator comprehension is faster than re.sub
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Performance optimization: "".join(str.split()) is faster than re.sub(r"\s+", "", raw)
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Performance optimization: replace chained strings and split is faster than re.split
