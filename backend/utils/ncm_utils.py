...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Bolt Optimization: generator comprehension is faster than regex for digit extraction
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Bolt Optimization: split() + join() is faster than regex for whitespace collapsing
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Bolt Optimization: chained replace() and split() is faster than re.split
