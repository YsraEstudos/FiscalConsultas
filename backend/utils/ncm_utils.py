...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Optimized: generator comprehension is faster than re.sub for retaining specific chars
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Optimized: str.split() followed by join is faster than re.sub for whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Optimized: chained replace + split is much faster than re.split with regex
