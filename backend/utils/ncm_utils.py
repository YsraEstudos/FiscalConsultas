...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # Opt: Generator comprehension is faster than regex for stripping non-numeric chars
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # Opt: "".join(str.split()) is faster than re.sub(r"\s+", "", str)
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # Opt: Chained replace and split is ~7x faster than re.split with multiple delimiters
