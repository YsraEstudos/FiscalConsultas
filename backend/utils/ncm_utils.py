...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # OPTIMIZATION: Generator comprehension checking against "0123456789" is faster
    # than re.sub and safer than str.isdigit() which allows unicode digits
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # OPTIMIZATION: "".join(s.split()) is faster than re.sub for whitespace removal
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # OPTIMIZATION: Chained .replace() followed by .split() is faster than re.split()
