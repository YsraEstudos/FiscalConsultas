...
def clean_ncm(ncm: str) -> str:
    ...
    """
    # ⚡ Bolt: Usando string join e set lookup ao invés de regex para performance.
...
def extract_chapter_from_ncm(ncm: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    raw = (ncm or "").strip()
    # ⚡ Bolt: Usando ''.join e .split() ao invés de regex sub("\s+") para melhor performance.
...
def split_ncm_query(query: str) -> List[str]:
    ...
    """
    # ⚡ Bolt: Usando chained replace e split() no lugar de re.split() para melhor performance.
