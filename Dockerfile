...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 CMD ["python", "-c", "import os,sys,urllib.request; port=os.environ.get('SERVER__PORT', '10000'); response=urllib.request.urlopen(f'http://127.0.0.1:{port}/api/status', timeout=3); sys.exit(0 if 200 <= response.status < 400 else 1)"]
