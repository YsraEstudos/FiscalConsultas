...
async def test_create_connection_wraps_connect_failures(monkeypatch):
    ...
    monkeypatch.setattr("backend.infrastructure.database.aiosqlite.connect", _fail_connect)
    monkeypatch.setattr("backend.infrastructure.database.logger.error", lambda *_args, **_kwargs: None)

    with pytest.raises(DatabaseError, match="Falha ao conectar ao banco: connect exploded"):
...
@pytest.mark.asyncio
async def test_detect_fts_schema_returns_unavailable_when_introspection_fails(monkeypatch):
...
async def test_get_chapter_notes_columns_returns_empty_set_on_error(monkeypatch):
    adapter = DatabaseAdapter("database/nesh.db")
    monkeypatch.setattr("backend.infrastructure.database.logger.warning", lambda *_args, **_kwargs: None)

    columns = await adapter._get_chapter_notes_columns(_ExplodingChapterNotesConnection())
...
@pytest.mark.asyncio
async def test_fts_search_near_returns_empty_list_when_query_execution_fails(monkeypatch):
    adapter = DatabaseAdapter("database/nesh.db")
    ...
    monkeypatch.setattr("backend.infrastructure.database.logger.debug", lambda *_args, **_kwargs: None)
