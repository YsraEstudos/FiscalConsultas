...
async def test_get_by_codigo_returns_scalar(monkeypatch):
    monkeypatch.setattr(
        "backend.infrastructure.repositories.tipi_repository.settings.database.engine",
        "sqlite",
    )
    expected = object()
...
async def test_fts_postgres_maps_rows(monkeypatch):
    ...
    assert out == [
        {"ncm": "8517", "capitulo": "85", "descricao": "Desc", "aliquota": "0"}
    ]
    _stmt, params = session.calls[0]
...
async def test_fts_sqlite_quotes_query(monkeypatch):
    ...
    assert out == [
        {"ncm": "8517", "capitulo": "85", "descricao": "Desc", "aliquota": "7"}
    ]
    _stmt, params = session.calls[0]
