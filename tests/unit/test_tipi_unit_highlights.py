...
def _make_pos(descricao: str, codigo: str = '84.14', ncm: str = '8414',
              aliquota: str = '10', nivel: int = 1) -> dict:
    return {'codigo': codigo, 'ncm': ncm, 'descricao': descricao,
            'aliquota': aliquota, 'nivel': nivel}


def _make_text_result(descricao: str, ncm: str = '8516.10',
                      capitulo: str = '85', aliquota: str = '5') -> list[dict]:
    return [{'ncm': ncm, 'capitulo': capitulo, 'descricao': descricao, 'aliquota': aliquota}]
...
class TestTipiRenderPositionUnits:
    ...
    @pytest.mark.parametrize("unit", ["kWh", "kVA", "Hz", "rpm", "MPa", "kg", "km", "ha"])
...
class TestTipiNewUnits:
    ...
    @pytest.mark.parametrize("unit", ["dB", "Ω", "kΩ", "ohm", "µF", "pF",
                                       "kN", "daN", "kJ", "MJ", "lm", "lux",
                                       "Btu", "psi"])
    ...
    def test_new_units_in_text_results(self, unit: str):
        ...
        assert _has_highlight(html, unit), f"'{unit}' deveria ser destacada em text_results"
