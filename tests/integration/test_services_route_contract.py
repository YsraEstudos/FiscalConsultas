...
class _FakeServicesCatalog:
    ...
    async def get_item_details(self, code: str):
        return {
            "success": True,
            "item": {"code": code, "description": "Serviços de construção"},
            "ancestors": [],
            "children": [],
            "nebs": None,
        }
    ...
    async def get_nebs_details(self, code: str):
        return {
            "success": True,
            "item": {"code": code, "description": "Serviços de construção de usinas de geração de energia"},
            "ancestors": [{"code": "1.0102.6", "description": "Serviços de construção de instalações industriais"}],
            "entry": {
                "code": code,
                "code_clean": "1010261",
                "title": "Serviços de construção de usinas de geração de energia",
...
@pytest.mark.parametrize(
    ("failure_reason", "expected_fragment"),
    [
        ("token expirado", "Token inválido ou expirado (token expirado)"),
        ("assinatura inválida", "Token inválido ou expirado (assinatura inválida)"),
    ],
)
def test_services_routes_reject_invalid_or_expired_tokens(
    client, monkeypatch, failure_reason, expected_fragment
):
    app.dependency_overrides[get_nbs_service] = lambda: _FakeServicesCatalog()
    ...
    monkeypatch.setattr(services_routes, "get_last_jwt_failure_reason", lambda: failure_reason)
    ...
    for endpoint in endpoints:
        ...
        assert response.status_code == 401
        assert response.json()["detail"] == expected_fragment
