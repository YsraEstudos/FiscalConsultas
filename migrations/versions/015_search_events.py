...
def upgrade() -> None:
    ...
    if "search_events" in inspector.get_table_names():
        existing_indexes = {
            index_name
    ...
    if "ix_search_events_user_created" not in existing_indexes:
        op.create_index(
            "ix_search_events_user_created",
            "search_events",
            ["user_id", "created_at"],
        )
    if "ix_search_events_tenant_created" not in existing_indexes:
        op.create_index(
            "ix_search_events_tenant_created",
            "search_events",
            ["tenant_id", "created_at"],
        )
    if "ix_search_events_fp_created" not in existing_indexes:
        op.create_index(
