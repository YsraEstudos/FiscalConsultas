...
import { DEFAULTS, SIDEBAR_POSITION, STORAGE_KEYS, VIEW_MODE } from '../../src/constants';
...
function SettingsProbe() {
    ...
    return (
        <div>
            ...
            <button onClick={() => settings.toggleHighlight()}>toggle-highlight</button>
            <button onClick={() => settings.toggleAdminMode()}>toggle-admin</button>
            <button onClick={() => settings.updateTipiViewMode(VIEW_MODE.FAMILY)}>tipi-family</button>
            <button onClick={() => settings.updateSidebarPosition(SIDEBAR_POSITION.RIGHT)}>sidebar-right</button>
            <button onClick={() => settings.restoreDefaults()}>restore-defaults</button>
        </div>
    );
}
...
describe('SettingsContext', () => {
    ...
    it('throws when useSettings is called outside provider', () => {
        ...
        expect(() => render(<OutsideConsumer />)).toThrow('useSettings must be used within a SettingsProvider');
    });
    ...
    it('loads persisted settings and applies highlight disabled classes', async () => {
        ...
        expect(screen.getByTestId('sidebar')).toHaveTextContent(SIDEBAR_POSITION.RIGHT);

        expect(document.documentElement.getAttribute('data-theme')).toBe('light');
        expect(document.documentElement.style.fontSize).toBe('18px');
        expect(document.body.classList.contains('disable-unit-highlights')).toBe(true);
        expect(document.body.classList.contains('disable-exclusion-highlights')).toBe(true);
    ...
    it('falls back to defaults for invalid persisted values and missing admin flag', async () => {
        ...
        expect(screen.getByTestId('tipi')).toHaveTextContent(DEFAULTS.TIPI_VIEW_MODE);
        expect(screen.getByTestId('sidebar')).toHaveTextContent(DEFAULTS.SIDEBAR_POSITION);
    });
    ...
    it('updates values and restores defaults', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('theme')).toHaveTextContent(DEFAULTS.THEME));
        expect(screen.getByTestId('font')).toHaveTextContent(String(DEFAULTS.FONT_SIZE));
        expect(screen.getByTestId('highlight')).toHaveTextContent(String(DEFAULTS.HIGHLIGHT));
        expect(screen.getByTestId('admin')).toHaveTextContent(String(DEFAULTS.ADMIN_MODE));
        expect(screen.getByTestId('tipi')).toHaveTextContent(DEFAULTS.TIPI_VIEW_MODE);
        expect(screen.getByTestId('sidebar')).toHaveTextContent(DEFAULTS.SIDEBAR_POSITION);
        expect(document.body.classList.contains('disable-unit-highlights')).toBe(false);
    });
