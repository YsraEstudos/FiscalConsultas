...
describe('useTabs', () => {
  ...
  it('starts with one empty nesh tab', () => {
    ...
    expect(result.current.activeTab.loadedChaptersByDoc).toEqual({ nesh: [], tipi: [] });
  ...
  it('creates tabs with deterministic id and switches active tab', () => {
    ...
    nowSpy.mockReturnValueOnce(171).mockReturnValueOnce(172);
  ...
  it('closes active tab selecting previous tab when available', () => {
    const nowSpy = vi.spyOn(Date, 'now');
    nowSpy.mockReturnValueOnce(501).mockReturnValueOnce(502);
    const { result } = renderHook(() => useTabs());
