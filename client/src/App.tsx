...
function App() {
  ...
  const handleOpenNoteRef = useRef<
    (note: string, chapter?: string) => Promise<void> | void
  >(() => {});
  const openTextResultInNewTabRef = useRef<
    (ncm: string, textQuery?: string) => Promise<void> | void
  >(() => {});
  ...
  const resetLoadedChaptersForDoc = useCallback((doc: DocType) => {
    const current = activeTabRef.current?.loadedChaptersByDoc || {
      nesh: [],
      tipi: [],
    };
    return { ...current, [doc]: [] };
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      const canReuseActiveTab =
        !currentTab?.loading && !currentTab?.results && !currentTab?.ncm;
      let startIndex = 0;
  ...
  const handleOpenNote = useCallback(
    async (note: string, chapter?: string) => {
      // NOSONAR
      const currentTab = activeTabRef.current;
      ...
      if (!targetChapter) {
        const fromQuery = extractChapter(
          currentTab?.ncm || results.query || "",
        );
      ...
      if (!content) {
        ...
        if (scrolled) {
          ...
          toast.error(
            `Nota ${note} não encontrada no capítulo ${targetChapter}.`,
          );
        }
        return;
      }
  ...
  const openInDocNewTab = useCallback(
    async (doc: DocType, ncm: string) => {
      const tabId = createTab(doc);
  ...
  const openTextResultInNewTab = useCallback(
    async (ncm: string, textQuery?: string) => {
  ...
  const openInDocCurrentTab = useCallback(
    async (doc: DocType, ncm: string) => {
      const currentTab = activeTabRef.current;
        ...
        <div className={styles.resultsSection}>
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              {(tab.loading ||
                (tab.results && tab.isContentReady === false)) && (
                <ResultSkeleton />
              )}
              ...
              {!tab.loading && tab.results && (
                <ResultDisplay
                  data={tab.results}
                  mobileMenuOpen={
                    tab.id === activeTabId ? mobileMenuOpen : false
                  }
                  onCloseMobileMenu={
                    tab.id === activeTabId ? closeMobileMenu : noop
                  }
                  onToggleMobileMenu={
                    tab.id === activeTabId
                  ...
                  onPersistScroll={(id, top) =>
                    updateTab(id, { scrollTop: top })
                  }
                  onContentReady={() => {
