...
const getAliquotClass = (aliquota: string) => {
  ...
  if (normalized === "NT") {
    return {
      className: "aliquot-nt",
      tooltip: "Não Tributável",
      display: "NT",
    };
  }
    ...
    .map((chapter: any) => {
      ...
      const positionsHtml = posicoes
        .map((pos: any) => {
          ...
          const { className, tooltip, display } = getAliquotClass(
            pos?.aliquota,
          );
...
export const ResultDisplay = React.memo(function ResultDisplay({
    ...
    for (const chapter of chapters) {
      ...
      for (const pos of positions) {
        ...
        if (
          !prefixMatch &&
          normalizedCodigo &&
          normalizedCodigo.startsWith(normalizedQuery)
        ) {
  ...
  const getSectionAnchorIdsFromResultados = useCallback((resultados: any) => {
    ...
    for (const chapter of chapters) {
      ...
      if (secoes && typeof secoes === "object") {
        const sectionTypes: ChapterSectionType[] = [
          "titulo",
          "notas",
          "consideracoes",
          "definicoes",
      ...
      if (
        ...
        (chapter?.notas_gerais || "").toString().trim()
      ) {
    ...
    try {
      ...
      if (!rawMarkup) {
        if (lastMarkupRef.current === cacheKey && lastHtmlRef.current) {
          ...
          rawMarkup = shouldParseMarkdown
            ? (marked.parse(markupToRender) as string)
            : markupToRender;
        }
        // Legacy compatibility: parse markdown only when markdown tokens are detected.
        // Primary path expects backend pure HTML.
      ...
      if (contentRef.current)
        contentRef.current.innerText = "Error rendering content.";
      renderedMarkupKeyRef.current = null;
      setIsContentReady(true);
    }
  ...
  useEffect(() => {
    ...
    const existing = targets.some((id) =>
      containerRef.current?.querySelector(`#${CSS.escape(id)}`),
    );
    if (existing) return;
    ...
    const formattedPos =
      normalizedPos.length >= 4
        ? `${normalizedPos.slice(0, 2)}.${normalizedPos.slice(2, 4)}`
        : candidateNcm;

    const byDataNcm = containerRef.current.querySelector(
      `[data-ncm="${formattedPos}"]`,
    ) as HTMLElement | null;
    if (byDataNcm) {
  ...
  useEffect(() => {
    if (!isActive || !isContentReady || !codeResults || !containerRef.current)
      return;
  ...
  return (
    <div className={wrapperClasses}>
      ...
      >
        ...
        >
          {!data.markdown && !isTipiResults(codeResults || null) && (
      ...
      <button
        ...
        aria-label={
          commentsEnabled ? "Desativar comentários" : "Ativar comentários"
        }
