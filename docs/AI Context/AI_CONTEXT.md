...
## 17) Ferramentas Disponiveis no Codex
...
### MCPs / integracoes principais

- `GitHub`: repositorios, issues, PRs, revisao e publicacao.
- `Linear`: issues, projetos, comentarios, docs, labels, milestones e pesquisa.
- `Neon`: projetos, branches, schemas, queries, migracoes, tuning e auth.
- `Render`: services, Postgres, logs, metrics, deploys, web services e cron jobs.
- `Upstash`: Redis, QStash, workflows, DLQ, schedules e estatisticas.
- `Playwright`: automacao de browser, validacao de UI e captura de evidencia.
- `tool_search`: descoberta de ferramentas dos conectores/apps quando necessario.

### Observacao

- A disponibilidade pode variar conforme a sessao e o workspace conectado.
